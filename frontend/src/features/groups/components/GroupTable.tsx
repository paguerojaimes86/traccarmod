...
function getSortValue(group: Group, key: string, groupMap: Map<number, string>): string {
  if (key === 'parent') return groupMap.get(group.groupId ?? -1) ?? '';
  const val = group[key as keyof Group];
...
export function GroupTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Group | null>(null);
  ...
  const columns = [
    ...
    { key: 'parent', label: 'Grupo Padre' },
  ];

  if (isLoading) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <LoadingState message="Cargando grupos..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar grupos" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((g) => (
            <tr key={g.id} style={rowStyle} className="group-row">
              ...
              <td style={tdStyle}>{groupMap.get(g.groupId ?? -1) ?? '—'}</td>
              {canManage && (
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar el grupo <strong style={{ color: '#0f172a' }}>{confirmItem.name}</strong>? Esta acción no se puede deshacer.
            </p>
