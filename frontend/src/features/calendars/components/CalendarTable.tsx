...
interface CalendarTableProps {
  ...
  onRetry?: () => void;
  canManage: boolean;
  onEdit: (item: Calendar) => void;
  onDelete: (item: Calendar) => void;
}
...
export function CalendarTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Calendar | null>(null);

  const filtered = useMemo(() => {
    if (!keyword.trim()) return items;
    const k = keyword.toLowerCase();
    return items.filter((c) => c.name?.toLowerCase().includes(k));
  }, [items, keyword]);
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <LoadingState message="Cargando calendarios..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar calendarios" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((c) => (
            <tr key={c.id} style={rowStyle} className="calendar-row">
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar el calendario <strong style={{ color: '#0f172a' }}>{confirmItem.name}</strong>? Esta acción no se puede deshacer.
            </p>
