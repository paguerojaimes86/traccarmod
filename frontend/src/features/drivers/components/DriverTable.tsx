...
export function DriverTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Driver | null>(null);

  const filtered = useMemo(() => {
    if (!keyword.trim()) return items;
    const k = keyword.toLowerCase();
    return items.filter((d) =>
      [d.name, d.uniqueId].some((v) => v?.toLowerCase().includes(k))
    );
  }, [items, keyword]);
  ...
  const columns = [
    ...
    { key: 'uniqueId', label: 'Identificador' },
  ];
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar conductores" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((d) => (
            <tr key={d.id} style={rowStyle} className="driver-row">
              ...
              <td style={tdStyle}>
                <div style={{ color: '#64748b', fontWeight: 600, fontFamily: 'SF Mono, monospace', fontSize: '0.8125rem' }}>{d.uniqueId}</div>
              </td>
              {canManage && (
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar el conductor <strong style={{ color: '#0f172a' }}>{confirmItem.name}</strong>? Esta acción no se puede deshacer.
            </p>
