...
export function CommandTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Command | null>(null);

  const filtered = useMemo(() => {
    if (!keyword.trim()) return commands;
    const k = keyword.toLowerCase();
    return commands.filter((c) =>
      [c.description, c.type].some((v) => v?.toLowerCase().includes(k))
    );
  }, [commands, keyword]);
  ...
  const columns = [
    ...
    { key: 'textChannel', label: 'Canal' },
  ];

  if (isLoading) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <LoadingState message="Cargando comandos..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar comandos" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((cmd) => (
            <tr key={cmd.id} style={rowStyle} className="command-row">
              <td style={{ ...tdStyle, borderTopLeftRadius: '1.125rem', borderBottomLeftRadius: '1.125rem', borderLeft: '1px solid rgba(15, 23, 42, 0.04)' }}>
                <div style={{ fontWeight: 800, color: '#0f172a' }}>{cmd.description || '—'}</div>
              </td>
              <td style={tdStyle}>
                <div style={pillStyle('#6366f1')}>{cmd.type}</div>
              </td>
              <td style={tdStyle}>{cmd.deviceId ? `ID: ${cmd.deviceId}` : 'Global'}</td>
              <td style={tdStyle}>
                <div style={pillStyle(cmd.textChannel ? '#f59e0b' : '#10b981')}>
                  {cmd.textChannel ? 'SMS' : 'Data'}
                </div>
              </td>
              {canManage && (
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar el comando <strong style={{ color: '#0f172a' }}>{confirmItem.description || confirmItem.type}</strong>? Esta acción no se puede deshacer.
            </p>
