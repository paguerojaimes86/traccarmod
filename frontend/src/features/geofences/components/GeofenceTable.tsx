...
function truncateArea(area: string | null | undefined): string {
  if (!area) return '—';
  const type = area.startsWith('POLYGON') ? 'POLYGON' : area.startsWith('CIRCLE') ? 'CIRCLE' : 'Área';
  return type;
}
...
export function GeofenceTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Geofence | null>(null);

  const filtered = useMemo(() => {
    if (!keyword.trim()) return geofences;
    const k = keyword.toLowerCase();
    return geofences.filter((g) =>
      [g.name, g.description, g.area].some((v) => v?.toLowerCase().includes(k))
    );
  }, [geofences, keyword]);
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <LoadingState message="Cargando geocercas..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar geocercas" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((gf) => (
            <tr key={gf.id} style={rowStyle} className="geofence-row">
              <td style={{ ...tdStyle, borderTopLeftRadius: '1.125rem', borderBottomLeftRadius: '1.125rem', borderLeft: '1px solid rgba(15, 23, 42, 0.04)' }}>
                <div style={{ fontWeight: 800, color: '#0f172a' }}>{gf.name}</div>
              </td>
              <td style={tdStyle}>{gf.description || '—'}</td>
              <td style={tdStyle}>
                <div style={pillStyle(gf.area?.startsWith('CIRCLE') ? '#8b5cf6' : '#0ea5e9')}>
                  {truncateArea(gf.area)}
                </div>
              </td>
              {canManage && (
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar la geocerca <strong style={{ color: '#0f172a' }}>{confirmItem.name}</strong>? Esta acción no se puede deshacer.
            </p>
