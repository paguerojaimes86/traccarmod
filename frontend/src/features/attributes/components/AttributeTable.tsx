...
export function AttributeTable({
  ...
  const [confirmItem, setConfirmItem] = useState<Attribute | null>(null);

  const filtered = useMemo(() => {
    if (!keyword.trim()) return items;
    const k = keyword.toLowerCase();
    return items.filter((a) =>
      [a.description, a.attribute].some((v) => v?.toLowerCase().includes(k))
    );
  }, [items, keyword]);
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <LoadingState message="Cargando atributos..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div style={{ padding: '4rem 0' }}>
        <ErrorState message="Error al cargar atributos" onRetry={onRetry} />
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={searchContainerStyle}>
        ...
        <input
          type="text"
          placeholder="Buscar por descripción, atributo..."
          value={keyword}
      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {pageItems.map((a) => (
            <tr key={a.id} style={rowStyle} className="attribute-row">
              <td style={{ ...tdStyle, borderTopLeftRadius: '1.125rem', borderBottomLeftRadius: '1.125rem', borderLeft: '1px solid rgba(15, 23, 42, 0.04)' }}>
                <div style={{ fontWeight: 800, color: '#0f172a' }}>{a.description}</div>
              </td>
              <td style={tdStyle}>
                <code style={{ fontSize: '0.8125rem', color: '#6366f1', fontWeight: 600 }}>{a.attribute}</code>
              </td>
              <td style={{ ...tdStyle, maxWidth: '280px' }}>
                <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '0.8125rem', color: '#64748b', fontFamily: 'monospace' }}>
                  {a.expression}
                </div>
              </td>
              <td style={tdStyle}>
                <div style={pillStyle(TYPE_COLORS[a.type?.toLowerCase() ?? ''] ?? '#64748b')}>
                  {a.type}
                </div>
              </td>
              <td style={tdStyle}>
                <span style={{ fontWeight: 700, color: '#0f172a' }}>{(a as unknown as { priority?: number }).priority ?? 0}</span>
              </td>
              {canManage && (
        ...
        <div style={overlayStyle} onClick={(e) => e.target === e.currentTarget && setConfirmItem(null)}>
          <div style={confirmCardStyle}>
            ...
            <p style={{ margin: '0 0 1.5rem', color: '#64748b', fontSize: '0.875rem', lineHeight: 1.5 }}>
              ¿Eliminar el atributo <strong style={{ color: '#0f172a' }}>{confirmItem.description}</strong>? Esta acción no se puede deshacer.
            </p>
